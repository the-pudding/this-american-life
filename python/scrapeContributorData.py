	...
	if (episodeNum != 374):
		...
		for act in soup2.findAll('div', class_='act'):
			if (act.find("h3")):
				...
				contributors = []
				if neighbor.find('ul', class_='act-contributors'):
					for c in neighbor.find('ul', class_='act-contributors').findAll('a'):
						contributors.append(c.get_text())
				else:
					contributors.append("NONE")
				actdata[str(actid)] = contributors
				num_acts += 1

with open('../important/contributorForEachAct.csv', "wb") as csv_file:
  writer = csv.writer(csv_file, delimiter=',')
  writer.writerow(["episode-act","contributor"])
